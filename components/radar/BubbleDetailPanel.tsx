    ...
    <aside className="w-full shrink-0 rounded-xl border border-slate-200 bg-slate-50 p-4 lg:w-72">
      ...
      <dl className="mt-3 space-y-2 text-sm">
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">綜合排序</dt>
          <dd className="font-semibold tabular-nums text-slate-800">{row.scoreS.toFixed(1)}</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">籌碼位置</dt>
          <dd className="tabular-nums">{fmt(row.flowScore)}</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">價位置</dt>
          <dd className="tabular-nums">{fmt(row.priceScore)}</dd>
        </div>
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">近5日法人</dt>
          <dd
            className={`tabular-nums ${row.net5dYi >= 0 ? 'text-rose-600' : 'text-emerald-700'}`}
          >
            {row.net5dYi >= 0 ? '+' : ''}
        ...
        <div className="flex justify-between gap-2">
          <dt className="text-slate-500">籌碼狀態</dt>
          <dd>{row.tideLabel}</dd>
        </div>
        <div className="flex justify-between gap-2">
